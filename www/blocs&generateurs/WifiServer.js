  ...
  init: function() {
    ...
    this.appendDummyInput()
   	...
   	this.appendValueInput("server_port")
        .setCheck("Number");
	this.appendDummyInput()
		.appendField(Blockly.Msg.WifiServer_ESP8266webserver_OTA);
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('Init the internal asyn web server in the defined port number in the block using the library ESPAsyncWebServer. You can not use these block if you are using other web server library');
    this.setHelpUrl('');
  }
};
...
Blockly.Arduino['wifiserver_port_asyn_library'] = function(block) {
  ...
  Blockly.Arduino.variables_['define_wifiserver_port'] = 'AsyncWebServer server('+server_port+');\n';
  
 
  var code = 'AsyncElegantOTA.begin(&server); //Start ElegantOTA \n'  
  +'server.begin();\n';
...
Blockly.Blocks["esp8266_asyngetArg"]={
	init:function(){
    this.setColour("#00AAAA");
   	this.appendDummyInput()
		.appendField(new Blockly.FieldImage("media/wifi.png",25,25))
...
Blockly.Arduino['esp8266_asyngetArg']=function(block){
	var arg=block.getFieldValue("arg");
    var code="String "+arg+"=request->arg(\""+arg+"\");\n"
	
	return code;
};


Blockly.Blocks["esp8266_asynuseArg"]={init:function(){
    this.setColour("#00AAAA");
    this.setHelpUrl(Blockly.Msg.esp8266_url);
   	this.appendDummyInput()
		.appendField(new Blockly.FieldImage("media/wifi.png",25,25))
...
Blockly.Arduino['esp8266_asynuseArg']=function(block){
    var arg=block.getFieldValue("arg");
    var code=arg;
...
Blockly.Blocks["esp8266_asynsend"]={init:function(){
    this.setColour("#00AAAA");
...
Blockly.Arduino['esp8266_asynsend']=function(block){
	
	var pagina=block.getFieldValue('text');
    var direccion=block.getFieldValue('address');
    var ordenes=Blockly.Arduino.statementToCode(block, "ORDERS");

	  Blockly.Arduino.definitions_["esp8266_asynserver"+pagina]='void serve'+pagina+'(AsyncWebServerRequest *request) {\n'
	  +"request->send(200,\"text/html\",p"+pagina+"());\n"+ordenes+"\n}\n";

	  Blockly.Arduino.setups_["esp8266_asynquery"+pagina]='server.on(\"/'+direccion+'\",HTTP_GET,serve'+pagina+');';
...
Blockly.Blocks["esp8266_asynsend_text"]={init:function(){
    this.setColour("#00AAAA");
	...
	this.appendDummyInput()
		...
		.appendField(new Blockly.FieldTextInput(""),"address");
	this.appendDummyInput()
		.appendField(Blockly.Msg.WifiServer_esp8266_send_text);
	this.appendValueInput("server_answer")
        .setCheck("String");
	this.setPreviousStatement(true);
    this.setNextStatement(true); 
...
Blockly.Arduino['esp8266_asynsend_text']=function(block){
	
	var direccion=block.getFieldValue('address');
	var answer_text = Blockly.Arduino.valueToCode(block, 'server_answer', Blockly.Arduino.ORDER_ATOMIC);
	//var ordenes=Blockly.Arduino.statementToCode(block, "ORDERS");
  
    Blockly.Arduino.setups_["esp8266_asynquery"+direccion]='server.on(\"/'+direccion+'\",HTTP_GET,[](AsyncWebServerRequest *request) {'
	  +'request->send(200, "text/plain",'+answer_text+' );});';
